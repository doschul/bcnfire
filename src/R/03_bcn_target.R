# ==============================================
# 03_bcn_target.R
# Description  : Define targeting variables/strategies and compute SAL costs/values.
# Inputs       : data/rdat/grd_filt.rds
# Outputs      : data/rdat/target_df.rds, data/rdat/summary_relative.rds, out/figures/sal_costs.png
...
summary(grd$sal_costs_ha)
summary(grd$sal_costs_cell)
...
rm_cols <- names(grd)[!names(grd) %in% c(names(grd)[1:22], "sal_costs_cell", 
                                         "geom", "inaccessibility", "cost_scaler")]
...
target_df_rdm <- target_df_agg %>%
  ...
  # drop groups
  ungroup() %>%
  dplyr::select(year, threshold, sum_SAL_costs) %>%
...
target_df_rel <- target_df_agg %>%
  ...
  dplyr::select(-rdm_sum_SAL_costs)
...
# descriptive: Average salvage logging cost per ha (variability over time and space)
library(tidyverse)
d1 <- target_df_agg %>%
  filter(strategy == "Random", threshold == 100) %>%
  mutate(
    total_area = tot_area,
    SAL_per_ha_eff = sum_SAL_costs / sum_SAL_ha,
    SAL_per_ha_tot = sum_SAL_costs / total_area
  )

quantile(d1$SAL_per_ha_tot, c(0, 0.25, 0.5, 0.75, 1))
