# ==============================================
# 02_data_prep.R
# Description  : Create regular grid, extract spatial covariates, filter to AOI, build neighbor list.
# Inputs       : data/raw/*, data/rdat/ignition_probability_layer.tif
# Outputs      : data/rdat/grd_filt.rds, data/rdat/neighbor_list.rds, out/figures/study_region_map.png
...
path_ignition_prob <- "./data/rdat/ignition_prob_rf.tif"
...
wui <- rast(path_wui)
rast_ign_prob <- rast(path_ignition_prob)
...
grd$inverse_road_distance <- 1 - (grd$road_distance / max_distance)

# Ensure all cells have a probability larger than zero
min_prob <- 0.01
grd$inverse_road_distance <- grd$inverse_road_distance + min_prob
grd$inverse_road_distance <- grd$inverse_road_distance / sum(grd$inverse_road_distance, na.rm = TRUE)

e_ign_prob <- exact_extract(rast_ign_prob, grd, fun = "mean")

#### Land cover and topography ####
lc_shares <- exact_extract(lc.rcl, grd, fun = "frac")
names(lc_shares) <- c("lc_agriculture", "lc_wildland", "lc_rock", 
                      "lc_artificial", "lc_water")

e.topo <- exact_extract(topo, grd, fun = c("mean", "max"))
names(e.topo) <- c("elev_mean",  "slope_mean", "elev_max", "slope_max") 

# wildland urba interface
wui_e <- exactextractr::exact_extract(wui, grd, fun="frac")

names(wui_e) <- c("wui_low", "wui_medium", "wui_nohouse", "wui_verylow", 
                  "wui_intermix", "wui_interface")
...
# fix values where BAU < SAL
# Create a logical SpatRaster identifying where BAU < SAL
condition_avg <- e.avg.bm[["biomass_bau_avg"]] < e.avg.bm[["biomass_sal_avg"]]
condition_avg[is.na(condition_avg)] <- FALSE 
table(condition_avg)

# 2. Perform the replacement: where the condition is TRUE, 
#    replace the BAU value with the corresponding SAL value.
e.avg.bm[["biomass_bau_avg"]][condition_avg] <- e.avg.bm[["biomass_sal_avg"]][condition_avg]

years_to_check <- 2040:2050
total_annual_replaced <- 0

for (year in years_to_check) {
  bau_name <- paste0("bau_ROS_", year)
  sal_name <- paste0("sal_ROS_", year)
  
  # Get the two layers for the current year
  bau_layer <- e.annual.ros[[bau_name]]
  sal_layer <- e.annual.ros[[sal_name]]
  
  # Create a logical raster for the replacement condition
  condition_annual <- bau_layer < sal_layer
  condition_annual[is.na(condition_annual)] <- FALSE 
  
  # Count replacements
  count_replaced_year <- sum(condition_annual)
  total_annual_replaced <- total_annual_replaced + count_replaced_year
  
  cat(paste("Year", year, ":", count_replaced_year, "values replaced.\n"))
  
  # Perform the replacement: assign SAL values to BAU cells where BAU < SAL
  # NOTE: We assign to the layer within the main object
  e.annual.ros[[bau_name]][condition_annual] <- sal_layer[condition_annual]
}
